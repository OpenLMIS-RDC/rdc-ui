    ...
    function factory($http, $q, openlmisUrlFactory, reportService, REPORTING_SERVICES,
        ...
        function getReportParamsOptions(report) {
            ...
            report.templateParameters.forEach(function(param) {
                ...
                if (param.selectExpression !== null && param.selectExpression !== undefined) {
        ...
        function getReportParamSelectExpressionUri(parameter, attributes) {
            ...
            if (attributes && dependencies.length > 0) {
                angular.forEach(dependencies, function(dep) {
                    var parentOption = attributes[dep.dependency];
                    if (!parentOption) {
                        return;
                    }
                    var sendValue = dep.property && parentOption.$raw
                        ? parentOption.$raw[dep.property]
                        : parentOption.value;
                    if (sendValue === undefined || sendValue === null || sendValue === '') {
                        return;
                    }
                    uri += (uri.indexOf('?') >= 0 ? '&' : '?')
                        + dep.placeholder + '=' + encodeURIComponent(sendValue);
                });
            }
            return uri;
        }
